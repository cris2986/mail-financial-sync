...
import { FinancialEvent, EventDirection, EventCategory, ScanSettings } from '../types';
...
const normalizeScanSettings = (settings?: Partial<ScanSettings>): ScanSettings => ({
  ...
  daysToScan: settings?.daysToScan ?? 90,
  enabledCategories: settings?.enabledCategories?.length
    ? settings.enabledCategories
    : ['card', 'credit', 'service', 'transfer', 'income']
});
...
const quoteForGmailQuery = (value: string): string => `"${value.replace(/"/g, '\\"')}"`;
...
export class GmailService {
  ...
  constructor(accessToken: string, scanSettings?: ScanSettings, onProgress?: ProgressCallback) {
    ...
    this.scanSettings = normalizeScanSettings(scanSettings || DEFAULT_SCAN_SETTINGS);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 403) {
            console.error('❌ Error 403: Permiso denegado o cuota excedida');
        ...
        console.error(`Gmail API request error (intento ${attempt + 1}):`, error);
        console.error(`Endpoint: ${endpoint}`);
        console.error(`Error type: ${error.constructor.name}`);
        
        if (error instanceof TypeError && error.message.includes('fetch')) {
          console.error('❌ Error de red: No se puede conectar con Gmail API');
          throw new Error('Network error: No se puede conectar con Gmail API');
        }
        
        if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
          console.error('❌ Error de conexión: Fallo al realizar la petición');
          throw new Error('Connection failed: Fallo al realizar la petición');
        }
  ...
  private getSendersToSearch(): string[] {
    ...
    if (this.scanSettings.useDefaultSenders) {
      senders.push(...FINANCIAL_SENDERS);
    }
  ...
  async searchFinancialEmails(maxResults = 500): Promise<GmailMessage[]> {
    ...
    // Construir query de búsqueda (poner entre comillas emisores con espacios)
    const senderQuery = senders.map(s => s.includes(' ') ? `from:"${s}"` : `from:${s}`).join(' OR ');
    ...
    for (let i = 0; i < allMessageIds.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map(msg =>
          this.request<GmailMessage>(`/messages/${msg.id}?format=full`)
        )
      );
  ...
  async getFinancialEvents(processedIds: string[] = []): Promise<FinancialEvent[]> {
    ...
    for (const message of messages) {
      ...
      const event = messageToEvent(message, allowedSenders, exclusionRules);
